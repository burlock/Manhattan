...
<body>
	...
	if($_POST['confirmNewPassword']){
		...
		if(($userRow['needPass']) && (!($userRow['passExpiration'] <= date('Y-m-d'))) && (!checkSimplePassChangeDE($_POST['newPassword'], $_POST['confirmNewPassword'], $keyError))){
		...
		elseif(!($userRow['needPass']) && ($userRow['passExpiration'] <= date('Y-m-d')) && (!checkHashedPassChangeDE($_POST['newPassword'], $_POST['confirmNewPassword'], $userRow['pass'], $keyError))){
		...
		elseif(($userRow['needPass']) && ($userRow['passExpiration'] <= date('Y-m-d')) && (!checkSimplePassChangeDE($_POST['newPassword'], $_POST['confirmNewPassword'], $keyError))){
		...
		elseif(!executeDBquery("UPDATE `users` SET `pass`='".$newCryptedPass."', `needPass`='0', `lastConnection` = CURRENT_TIMESTAMP, `passExpiration`='".addMonthsToDate(getDBsinglefield('value', 'otherOptions', 'key', 'expirationMonths'))."' WHERE `login`='".$_SESSION['loglogin']."'")){
			...
			<div class="top-alert-container">
				<div class="alert alert-success top-alert fade in">
					<a href="#" class="close" data-dismiss="alert">&times;</a>
					<strong>Éxito!</strong> Aktualisierte passwort.
				</div>
			</div>					
